...
class TelnetBruteforce:
    ...
    def execute(self, ip, port, row, status_key):
        ...
        """
        if self.telnet_connector.disabled:
            logger.warning("Telnet actions disabled (telnetlib not available).")
            return 'failed'
...
class TelnetConnector:
    ...
    def __init__(self, shared_data):
        self.shared_data = shared_data
        self.disabled = telnetlib is None
        if self.disabled:
            logger.warning("telnetlib not available; skipping TelnetConnector setup.")
            return
